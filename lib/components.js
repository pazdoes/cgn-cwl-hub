...
export function RankBadge({ rank }) {
  ...
  if (colours[rank]) {
    const { stroke, bg, border } = colours[rank];
    return (
      <span style={{background: bg, border: `1px solid ${border}`}} className="inline-flex items-center justify-center w-5 h-5 rounded-full">
        <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke={stroke} strokeWidth={1.8}>
