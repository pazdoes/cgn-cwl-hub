...
export function RankBadge({ rank }) {
  const medals = {
    1: { bg: "#F59E0B", ring: "#D97706", text: "#fff" },
    2: { bg: "#94A3B8", ring: "#64748B", text: "#fff" },
    3: { bg: "#B45309", ring: "#92400E", text: "#fff" },
  };
  if (medals[rank]) {
    const { bg, ring, text } = medals[rank];
    return (
      <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle cx="11" cy="11" r="10" fill={bg} stroke={ring} strokeWidth="1.5"/>
        <text x="11" y="15" textAnchor="middle" fontSize="10" fontWeight="bold" fill={text}>{rank}</text>
      </svg>
    );
  }
