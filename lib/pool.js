...
// ── Item 26: CWL season history ──────────────────────────────────────────────

// Records the current CWL rank for every clan in a given season.
// Called once per season by an admin via the "Record Season" button.
// Uses INSERT ... ON CONFLICT DO UPDATE so it's safe to call multiple
// times — re-recording a season just overwrites the previous value,
// which is the right behaviour if an admin records too early then wants
// to correct it after CWL results are finalised.
...
// Returns all history rows ordered by clan then by season. The frontend
// is responsible for sorting seasons chronologically and interpolating
// missing seasons (carry-forward with dashed line rendering).
