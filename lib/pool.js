...
export async function scheduleAnnouncement({ webhookId, embedJson, content, username, avatarUrl, sendAt, createdBy, title }) {
  const sql = getDb();
  const rows = await sql`
    INSERT INTO scheduled_announcements
      (webhook_id, title, embed_json, content, username, avatar_url, send_at, created_by)
    VALUES (
      ${webhookId},
      ${title || null},
