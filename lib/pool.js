...
export async function updateClanSeasonStats(season, clanStatsMap) {
  ...
  for (const [clanName, cs] of Object.entries(clanStatsMap)) {
    ...
    const atkEff = attacksUsed > 0 ? parseFloat((cs.totalStars / attacksUsed).toFixed(2)) : null;
    const defEff = attacksAvail > 0 ? parseFloat((cs.totalStarsConceded / attacksAvail).toFixed(2)) : null;
    const threeStarRate = cs.totalAttacksForRate > 0 ? parseFloat(((cs.threeStars / cs.totalAttacksForRate) * 100).toFixed(2)) : null;
    const avgDest = attacksUsed > 0 ? parseFloat((cs.destSum / attacksUsed).toFixed(2)) : null;
    const avgDef = cs.defCount > 0 ? parseFloat((cs.defSum / cs.defCount).toFixed(2)) : null;
    await sql`
      UPDATE clan_season_history SET
        ...
        total_stars_conceded   = ${cs.totalStarsConceded},
