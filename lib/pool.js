...
// "In the pool" means "has a pool_entries row that isn't an explicit Out
// declaration" — not merely "a row exists". Setting cwl_intent to 'out'
// still requires a row to exist (that's the only place intent can be
// stored), so treating row-existence alone as "in pool" would make anyone
// who's explicitly opted out look like they're back in the moment their
// intent is recorded. IS DISTINCT FROM 'out' is null-safe: a row with no
// explicit intent (the normal join case) still correctly counts as in
// pool, and any future intent value other than the literal string 'out'
// defaults to "in" too, rather than silently excluding people.
export async function isInPool(playerTag, season) {
  ...
  const rows = await sql`
    ...
    WHERE player_tag = ${playerTag} AND season = ${season}
      AND cwl_intent IS DISTINCT FROM 'out'
    LIMIT 1
  `;
    ...
    SELECT at.*, dw.label AS webhook_label,
      COALESCE(
        (SELECT json_agg(json_build_object('id', tg.id, 'name', tg.name) ORDER BY tg.name)
         FROM announcement_tags tg WHERE tg.id = ANY(at.tag_ids)),
        '[]'
      ) AS tags
...
export async function updateAnnouncementTemplate(id, { name, webhookId, embedJson, username, avatarUrl, tagIds }) {
  const sql = getDb();
  const rows = await sql`
    UPDATE announcement_templates
    SET name = ${name}, webhook_id = ${webhookId || null}, embed_json = ${JSON.stringify(embedJson)},
        username = ${username || null}, avatar_url = ${avatarUrl || null}, tag_ids = ${tagIds || []}
    WHERE id = ${id}
    RETURNING *
  `;
  return rows[0];
...
export async function saveAnnouncementTemplate({ name, webhookId, embedJson, username, avatarUrl, tagIds }) {
  const sql = getDb();
  const rows = await sql`
    INSERT INTO announcement_templates (name, webhook_id, embed_json, username, avatar_url, tag_ids)
    VALUES (${name}, ${webhookId || null}, ${embedJson}, ${username || null}, ${avatarUrl || null}, ${tagIds || []})
    RETURNING *
  `;
  return rows[0];
}
...
// ── Announcement template tags ───────────────────────────────────────────────
// A managed list, not free-text per template — tags are only ever created in
// one deliberate place (the Tools tab) and only ever selected (not typed)
// everywhere else, so a stray case-variant duplicate ("cwl" vs "CWL") can't
// happen through normal use. The unique index on LOWER(name) backs that up
// at the database level too, not just in the application.
export async function getAnnouncementTags() {
  const sql = getDb();
  return sql`
    SELECT tg.id, tg.name,
      (SELECT COUNT(*) FROM announcement_templates t WHERE tg.id = ANY(t.tag_ids))::int AS template_count
    FROM announcement_tags tg
    ORDER BY tg.name
  `;
}

export async function createAnnouncementTag(name) {
  const sql = getDb();
  const trimmed = name.trim();
  const [existing] = await sql`
    SELECT id, name FROM announcement_tags WHERE LOWER(name) = LOWER(${trimmed})
  `;
  if (existing) {
    const err = new Error(`A tag called "${existing.name}" already exists`);
    err.code = "DUPLICATE_TAG";
    throw err;
  }
  const [tag] = await sql`
    INSERT INTO announcement_tags (name) VALUES (${trimmed}) RETURNING id, name
  `;
  return { ...tag, template_count: 0 };
}

export async function deleteAnnouncementTag(id) {
  const sql = getDb();
  // Remove the reference from any templates first, so nothing is left
  // pointing at a tag that no longer exists, then delete the tag itself.
  await sql`
    UPDATE announcement_templates SET tag_ids = array_remove(tag_ids, ${id}) WHERE ${id} = ANY(tag_ids)
  `;
  await sql`DELETE FROM announcement_tags WHERE id = ${id}`;
}
...
export async function upsertOutsideAllianceCache({ outsideAllianceCount }) {
  const sql = getDb();
  await sql`
    INSERT INTO admin_live_checks_cache (id, outside_alliance_count, checked_at)
    VALUES (1, ${outsideAllianceCount}, now())
    ON CONFLICT (id) DO UPDATE SET
      outside_alliance_count = EXCLUDED.outside_alliance_count,
      checked_at              = EXCLUDED.checked_at
  `;
}
