...
export async function logAnnouncement(webhookId, title, embedJson, sentBy) {
  ...
  await sql`
    ...
    VALUES (${webhookId}, ${title || null}, ${embedJson}, ${sentBy || null})
  `;
}
  ...
  for (const a of attacks) {
    await sql`
      INSERT INTO war_attacks (
        ...
        ${a.warResult ?? null}, ${a.opponentClan ?? null}
      )
...
export async function upsertWarDay(warDay) {
  ...
  await sql`
    INSERT INTO war_days (
      ...
      ${warDay.warResult ?? null}, ${warDay.opponentClan ?? null}
    )
...
/**
 * Bulk update player_cwl_stats with computed war metrics.
 */
export async function updatePlayerWarMetrics(season, metrics) {
  const sql = getDb();
  for (const m of metrics) {
    await sql`
      UPDATE player_cwl_stats SET
        avg_stars_per_attack = ${m.avg_stars_per_attack ?? null},
        three_star_rate       = ${m.three_star_rate ?? null},
        punch_up_rate         = ${m.punch_up_rate ?? null},
        clutch_rate           = ${m.clutch_rate ?? null},
        consistency_score     = ${m.consistency_score ?? null},
        dips                  = ${m.dips ?? null},
        reaches               = ${m.reaches ?? null}
      WHERE season = ${season}
        AND player_tag = ${m.player_tag}
    `;
  }
}

/**
 * Upsert per-day war performance rows for player profile curves.
 */
export async function upsertPlayerWarDayStats(rows) {
  if (!rows?.length) return;
  const sql = getDb();
  for (const r of rows) {
    await sql`
      INSERT INTO player_war_day_stats (
        player_tag, season, clan_name, war_day,
        stars, destruction_pct, war_result, opponent_clan
      ) VALUES (
        ${r.playerTag}, ${r.season}, ${r.clanName}, ${r.warDay},
        ${r.stars ?? null}, ${r.destructionPct ?? null},
        ${r.warResult ?? null}, ${r.opponentClan ?? null}
      )
      ON CONFLICT (player_tag, season, war_day) DO UPDATE SET
        stars = EXCLUDED.stars,
        destruction_pct = EXCLUDED.destruction_pct,
        war_result = EXCLUDED.war_result,
        opponent_clan = EXCLUDED.opponent_clan
    `;
  }
}
