...
// CWL Rank) from Neon. Used by assignPlayerToRoster as a last-resort
// fallback after the Sheet-only carry-forward finds nothing to borrow
// from — i.e. specifically for a brand-new clan's first-ever assigned
// player. Returns nulls for any field not set, which the caller treats
// the same as "no value available" (falls through to empty string).
export async function getClanFallbackData(clanName) {
  const sql = getDb();
  const rows = await sql`
    SELECT clan_tag, clan_link, cwl_rank FROM clans WHERE clan_name = ${clanName}
  `;
  return rows[0] ?? { clan_tag: null, clan_link: null, cwl_rank: null };
}
