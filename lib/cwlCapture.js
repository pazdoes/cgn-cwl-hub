import { getAllClanNames, getAllClanFormats, getClanFallbackData, recordSeasonHistory, upsertPlayerCwlStats, updateClanSeasonStats, upsertWarAttacks, upsertWarDay, getCapturedWarTags, getSeasonWarAttacksByPlayer } from "@/lib/pool";
  ...
  for (const clanName of clanNames) {
    try {
      ...
      for (const row of dbAttacks) {
        ...
        if (ps._seenWarTags.has(row.war_tag)) continue;
        ps._seenWarTags.add(row.war_tag);
        ps.attacksAvailable += 1;
        ps.attacksUsed += row.stars !== null ? 1 : 0;
