...
export async function captureCwlData(season) {
  ...
  for (const [clanName, cs] of Object.entries(clanStatsMap)) {
    ...
    const maxStars   = maxAttacks * 3;      // e.g. 105×3=315 or 210×3=630
    const maxWars    = 7;
    ...
    // Don't cap totalAttacksAvailable — it comes directly from API and reflects actual team size
    // Only cap totalAttacksUsed and totalStars as sanity bounds
    cs.totalAttacksUsed   = Math.min(cs.totalAttacksUsed, cs.totalAttacksAvailable);
    cs.totalAttacksMissed = Math.max(0, cs.totalAttacksAvailable - cs.totalAttacksUsed);
