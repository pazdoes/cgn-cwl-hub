...
export async function captureCwlData(season) {
  ...
  for (const clanName of clanNames) {
    try {
      ...
      // ── Roster-based reconciliation of attacksAvailable / missedAttacks ──
      // Runs over EVERY fetchable war (inWar or warEnded), not just inWar.
      // This is the sole source of truth for these two fields — separate from
      // starsEarned/attacksUsed/star-breakdowns above, which continue to come
      // from the DB seed (for warEnded wars) or the live merge (for inWar).
      //
      // Why this exists: a missed attack never produces a war_attacks row, so
      // seeding attacksAvailable from war_attacks (as the DB-seed step above
      // does for stars/attacksUsed) can never detect a miss — it silently
      // undercounts attacksAvailable down to whatever the player actually
      // attacked. Scanning each war's full member list directly, regardless
      // of whether that war has already ended, is the only way to know a
      // player was rostered for a round even if they never swung.
      // _seenRosterWarTags (distinct from _seenWarTags) prevents this from
      // double-counting across multiple cron runs of this function.
      for (const war of wars) {
        if (!war) continue;
        const warTag = war.tag;
        const ourSide = war.clan?.tag === clanTag ? war.clan
        ...
        for (const member of ourSide.members || []) {
          ...
          const ps = allPlayerStats[tag];
          if (!ps._seenRosterWarTags) ps._seenRosterWarTags = new Set();
          if (ps._seenRosterWarTags.has(warTag)) continue;
          ps._seenRosterWarTags.add(warTag);
          ps.attacksAvailable += 1;
          if ((member.attacks || []).length === 0) ps.missedAttacks += 1;
        }
      }
      clansProcessed++;
    } catch (err) {
