import { getAllClanNames, getAllClanFormats, getClanFallbackData, recordSeasonHistory, upsertPlayerCwlStats, updateClanSeasonStats, upsertWarAttacks, upsertWarDay, getCapturedWarTags, updatePlayerWarMetrics, upsertPlayerWarDayStats } from "@/lib/pool";
...
export async function captureCwlData(season) {
  ...
  // Compute and store per-war metrics from war_attacks table
  try {
    await computePlayerWarMetrics(season);
  } catch (err) {
    errors.push(`War metrics: ${err.message}`);
  }
  ...
  for (const clanName of clanNames) {
    try {
      ...
      for (const entry of wars) {
        ...
        // Build defender TH level and map position lookup
        const defenderThMap = {};
        const defenderMapMap = {};
        for (const member of [...(war.clan?.members || []), ...(war.opponent?.members || [])]) {
          ...
          defenderMapMap[member.tag] = member.mapPosition ?? null;
        }
        ...
        for (const member of ourSide.members || []) {
          ...
          // Build a map of best stars already on each defender before this attack
          // so we can compute true stars (net new stars earned)
          const priorStarsOnBase = {};

          for (let i = 0; i < attacks.length; i++) {
            const atk = attacks[i];
            const priorBest = priorStarsOnBase[atk.defenderTag] ?? 0;
            const trueStars = Math.max(0, atk.stars - priorBest);
            priorStarsOnBase[atk.defenderTag] = Math.max(priorBest, atk.stars);
            ...
            attackRows.push({
              ...
              attackerMapPosition: member.mapPosition ?? null,
              defenderMapPosition: defenderMapMap[atk.defenderTag] ?? null,
              warResult,
              opponentClan,
            });
...
/**
 * Computes per-war metrics from war_attacks and stores them in player_cwl_stats.
 * Also populates player_war_day_stats for per-day performance curves.
 * Uses PostgreSQL aggregate functions for efficiency.
 */
export async function computePlayerWarMetrics(season) {
  const { getDb } = await import("@/lib/db");
  const sql = getDb();

  // Compute all metrics in one query using PostgreSQL aggregates
  const metrics = await sql`
    SELECT
      player_tag,
      clan_name,
      -- Avg stars per attack (only counting attacks made)
      ROUND(AVG(stars)::NUMERIC, 2) AS avg_stars_per_attack,
      -- 3-star rate as percentage
      ROUND((COUNT(*) FILTER (WHERE stars = 3) * 100.0 / NULLIF(COUNT(*), 0))::NUMERIC, 2) AS three_star_rate,
      -- Punch-up rate: attacks against equal or higher TH
      ROUND((COUNT(*) FILTER (WHERE defender_th_level > town_hall_level AND town_hall_level IS NOT NULL AND defender_th_level IS NOT NULL) * 100.0 / NULLIF(COUNT(*), 0))::NUMERIC, 2) AS punch_up_rate,
      -- Clutch rate: avg stars on war days 5-7
      ROUND(AVG(stars) FILTER (WHERE war_day >= 5)::NUMERIC, 2) AS clutch_rate,
      -- Consistency: standard deviation of stars (lower = more consistent)
      ROUND(STDDEV(stars)::NUMERIC, 2) AS consistency_score,
      -- Dips and reaches counts
      COUNT(*) FILTER (WHERE defender_th_level < town_hall_level AND town_hall_level IS NOT NULL AND defender_th_level IS NOT NULL) AS dips,
      COUNT(*) FILTER (WHERE defender_th_level > town_hall_level AND town_hall_level IS NOT NULL AND defender_th_level IS NOT NULL) AS reaches
    FROM war_attacks
    WHERE season = ${season}
    GROUP BY player_tag, clan_name
  `;

  if (metrics.length > 0) {
    await updatePlayerWarMetrics(season, metrics);
  }

  // Compute per-day breakdown per player
  const dayStats = await sql`
    ...
    FROM war_attacks
    WHERE season = ${season}
    ORDER BY player_tag, war_day
  `;

  if (dayStats.length > 0) {
    const rows = dayStats.map(r => ({
      playerTag: r.player_tag,
      season,
      clanName: r.clan_name,
      warDay: r.war_day,
      stars: r.stars,
      destructionPct: r.destruction_pct,
      warResult: r.war_result,
      opponentClan: r.opponent_clan,
    }));
    await upsertPlayerWarDayStats(rows);
  }

  return { playersComputed: metrics.length, dayRowsComputed: dayStats.length };
}
