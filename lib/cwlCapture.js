...
export async function captureCwlData(season) {
  ...
  for (const clanName of clanNames) {
    try {
      ...
      // Collect war tags not yet processed
      const warTags = [];
      ...
      // Mark all as processed immediately before parallel fetch
      // prevents other clan loops re-fetching the same wars
      for (const wt of warTags) processedWarTags.add(wt);

      // Fetch all wars in parallel — dramatically reduces execution time
      const wars = await Promise.all(
        warTags.map(wt => getCwlWar(wt).catch(() => null))
      );
