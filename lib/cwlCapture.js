  ...
  for (const clanName of clanNames) {
    try {
      ...
      for (const warTag of warTags) {
        ...
        console.log(`[CWL DEBUG] warTag=${warTag} clan=${clanName} members=${ourSide.members?.length}`);
        for (const member of ourSide.members || []) {
          ...
          if (ps._seenWarTags.has(warTag)) {
            console.log(`[CWL DEBUG] SKIPPING ${tag} for warTag=${warTag} (already seen)`);
            continue;
          }
