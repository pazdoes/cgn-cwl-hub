...
export async function captureCwlData(season) {
  ...
  const statsToSave = Object.values(allPlayerStats).map(ps => ({
    ...
    missedAttacks: Math.min(ps.missedAttacks, Math.min(ps.attacksAvailable, MAX_CWL_ATTACKS)),
    attacksUsed: Math.max(0, Math.min(ps.attacksAvailable, MAX_CWL_ATTACKS) - Math.min(ps.missedAttacks, Math.min(ps.attacksAvailable, MAX_CWL_ATTACKS))),
