...
export async function clearRosterAssignments() {
  ...
  for (const tab of clanTabs) {
    ...
    // Build clear requests — blank player-specific columns for every data row
    // Preserve: CLAN_LINK (G), CWL_FORMAT (H), CWL_RANK (I), CLAN_TAG (K)
    const clearRequests = [];
    for (let i = 1; i < rows.length; i++) {
      const rowNum = i + 1; // 1-indexed, skip header
      // Clear: A (Position), B (Account), C (Player Tag), D (Clan), E (Town Hall), F (Status), J (Season)
      clearRequests.push(`${encodeURIComponent(tab)}!A${rowNum}:F${rowNum}`);
      clearRequests.push(`${encodeURIComponent(tab)}!J${rowNum}`);
    }

    // Batch clear
    for (const range of clearRequests) {
      await fetch(
