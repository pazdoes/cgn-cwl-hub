...
export async function assignPlayerToRoster({ tag, playerName, clan, townHall, season }) {
  ...
  // not per-player — assigning a player must never blank them out. When
  // updating an existing row, carry the current values forward unchanged.
  // When appending a brand new row, leave them blank — they'll be filled
  // in by the clan's existing format (set via the admin format toggle,
  // item 5) or left for an officer to fill in for CWL Rank specifically.
  const existingFormat = existingRowIndex >= 0 ? (rows[existingRowIndex][COL.CWL_FORMAT] || "") : "";
  const existingRank   = existingRowIndex >= 0 ? (rows[existingRowIndex][COL.CWL_RANK]   || "") : "";
