...
// Clears every clan tab's roster columns in one batched request per tab,
// instead of firing two unbatched requests per data row. A clan with 15
// rostered players previously meant 30 separate write calls; Google
// Sheets enforces a per-minute write quota, and once earlier clans in the
// loop used it up, later clans' clear requests could fail silently — the
// old code never checked each request's response, so a failed clear
// looked identical to a successful one. That's exactly how a season
// migration could leave some clans' tabs with last season's roster still
// showing while others correctly cleared. Returns which clans (if any)
// failed to clear, so that can be surfaced to whoever ran the migration
// instead of only being visible if someone happens to check server logs.
export async function clearRosterAssignments() {
  ...
  // Skip non-clan tabs (Ranking, Rounds etc.)
  const clanTabs = tabs.filter(t =>
    !t.toLowerCase().includes("ranking") &&
  ...
  for (const tab of clanTabs) {
    const rows = await getSheetValues(token, tab);
    if (rows.length <= 1) continue; // header only

    // Build the same ranges as before — blank player-specific columns for
    // every data row. Preserve: CLAN_LINK (G), CWL_FORMAT (H), CWL_RANK (I),
    // CLAN_TAG (K). Now sent as ONE batchClear request instead of one
    // request per range. Tab name is single-quoted per Sheets' A1 notation
    // rules for names containing spaces/special characters (matching the
    // same convention already used in lib/sheets.js's batchGet ranges) —
    // these go straight into a JSON body with no URL encoding to fall back
    // on, unlike the old per-range :clear calls.
    const ranges = [];
    for (let i = 1; i < rows.length; i++) {
      const rowNum = i + 1; // 1-indexed, skip header
      // Clear: A (Position), B (Account), C (Player Tag), D (Clan), E (Town Hall), F (Status), J (Season)
      ranges.push(`'${tab}'!A${rowNum}:F${rowNum}`);
      ranges.push(`'${tab}'!J${rowNum}`);
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error(`clearRosterAssignments: failed to clear "${tab}" (${res.status}): ${errText}`);
        failed.push(tab);
      }
    } catch (err) {
      console.error(`clearRosterAssignments: request failed for "${tab}":`, err);
      failed.push(tab);
    }
  }
...
export async function createClanTab(clanName) {
  ...
  console.log("createClanTab: starting for", clanName);
  let token;
  try {
    token = await getAccessToken();
    console.log("createClanTab: got access token");
  } catch (err) {
    console.error("createClanTab: getAccessToken failed:", err.message);
    throw err;
  }

  let existingTabs;
  try {
    existingTabs = await getSheetTabs();
    console.log("createClanTab: existing tabs:", existingTabs);
  } catch (err) {
    console.error("createClanTab: getSheetTabs failed:", err.message);
    throw err;
  }
  ...
  if (!addRes.ok) {
    ...
    console.error("Sheet batchUpdate raw error:", addRes.status, errBody);
    let err = {};
    try { err = JSON.parse(errBody); } catch {}
    throw new Error(err?.error?.message || `Sheet tab creation failed: ${errBody || addRes.status}`);
  }
