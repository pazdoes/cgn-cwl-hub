...
export async function assignPlayerToRoster({ tag, playerName, clan, townHall, season }) {
  ...
  // rather than per-player). If the tab has no other rows yet (a brand
  // new, completely empty clan tab), all four fall back to "" and an
  // admin will need to set them via their respective controls (format
  // toggle, rank refresh, or manually for Clan Link/Clan Tag) once at
  // least one player is assigned.
  function carryForward(col) {
    if (existingRowIndex >= 0) {
      return rows[existingRowIndex][col] || "";
    }
    const anyOtherRow = rows.find((r, i) => i > 0 && (r[col] || "").trim() !== "");
    return anyOtherRow ? anyOtherRow[col] : "";
  }

  const existingClanLink = carryForward(COL.CLAN_LINK);
  const existingFormat   = carryForward(COL.CWL_FORMAT);
  const existingRank     = carryForward(COL.CWL_RANK);
  const existingClanTag  = carryForward(COL.CLAN_TAG);
